...
class Solution:
    def frogJump(self,ind, heights):
        ...
        for i in range(1,ind + 1): # whatver part were doing something on ind repeatedly --> make that to i na inlcude in for loop
            ...
            curi = min(fs,ss) # after calculating --> update the values for prev and prev2 --> beacuse i will be shifted to next step

            prev2 = prev
            prev = curi

        return prev # by the time loop exits -> when i = n, final value already stored in n-1 index ==> that is prev
